...
if (isset($_POST['submit'])) {
    ...
    $result = mysqli_query($con, "DELETE FROM request WHERE email = '" . $email. "' and id = '" . $id . "'");
    ...
    <ul class="nav navbar-nav navbar-right">
     ...
     <table class="table table-bordered table-hover" >
       ...
       <tbody>
         ...
         <?php
             $result = mysqli_query($con, "SELECT * FROM request WHERE value= '1'");
             ...
             ?>
                 <tr>
                     ...
                     <td>
                    <form  action="allocation.php" method="get">
                      <input type="hidden" name="name" value="<?php echo $row['name'] ?>" />
                      <input type="hidden" name="email" value="<?php echo $row['email'] ?>" />
                      <input type="hidden" name="designation" value="<?php echo $row['designation'] ?>" />
                      <input type="hidden" name="guestno" value="<?php echo $row['no_of_guest'] ?>" />
                      <input type="hidden" name="location" value="<?php echo $row['location'] ?>" />
                      <input type="hidden" name="datefrom" value="<?php echo $row['datefrom'] ?>" />
                      <input type="hidden" name="dateto" value="<?php echo $row['dateto'] ?>" />
                      <input type="hidden" name="id" value="<?php echo $row['id'] ?>" />
                       <button type="submit" name="submit" value="Submit" class="btn btn-primary">Allocate</button>
                     </form>
                     </td>
                     <td>
